...
def bfs(board, start_node, goal_node):
    # 방문 여부를 카운팅 할 배열
    ...
    while need_visit:
        ...
        if row > 0:
            if visited[row - 1][col] == 0 and board[row - 1][col] == 0:
                need_visit.append(path + [[row - 1, col]])
                visited[row - 1][col] = 1

        if col > 0:
            if visited[row][col - 1] == 0 and board[row][col - 1] == 0:
                need_visit.append(path + [[row, col - 1]])
                visited[row][col - 1] = 1
...
if __name__ == "__main__":
    ...
    start_node = [0, 0]
    goal_node = [5, 4]
    # bfs 통해 시작 노드에서 도착 노드까지의 최단 경로 리턴
    print(bfs(board, start_node, goal_node))
